"""
PNN_Scene_Recognition.py
=========================
Replicates the three scene recognition tasks described in the Advanced Optical
Materials paper for the PNN (Nonvolatile Photonic Neural Network) system,
...
Usage:
  python PNN_Scene_Recognition.py --task all      # run all three tasks
  python PNN_Scene_Recognition.py --task speech   # speech only
  python PNN_Scene_Recognition.py --task fashion  # Fashion-MNIST only
  python PNN_Scene_Recognition.py --task mnist    # MNIST only
"""
...
def generate_results(output_dir, history, task_name):
    ...
    # 4. Confusion matrix – PNN (after LUT quantization)
    plot_confusion_matrix(
        history['pnn_cm'], class_names,
        f'Confusion Matrix – {title_name} (PNN with LUT)',
        os.path.join(output_dir, 'confusion_matrix_pnn.png'),
        cmap='Blues',
    )
    ...
    ax.bar(x + w / 2, history['pnn_digit_accs'] * 100, w,
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(output_dir, 'per_class_accuracy.png'), dpi=_PLOT_DPI)
    plt.close(fig)

    # 6. Text report
    report_path = os.path.join(output_dir, 'final_recognition_report.txt')
    with open(report_path, 'w') as fh:
        fh.write(f"=== PNN Scene Recognition – {title_name} ===\n\n")
        fh.write(f"Full Precision  (64-bit Computer): {history['computer_accuracy'] * 100:.2f}%\n")
        fh.write(f"PNN Simulation (LUT Quantized):  {history['pnn_accuracy'] * 100:.2f}%\n")
        fh.write(f"Accuracy drop after PTQ:           "
                 f"{(history['computer_accuracy'] - history['pnn_accuracy']) * 100:.2f}%\n\n")
        fh.write("Per-Class Accuracy – Full Precision:\n")
        for i, cls in enumerate(class_names):
            fh.write(f"  Class {i:2d} ({cls:10s}): {history['computer_digit_accs'][i] * 100:.2f}%\n")
        fh.write("\nPer-Class Accuracy – PNN (LUT Quantized):\n")
        for i, cls in enumerate(class_names):
            fh.write(f"  Class {i:2d} ({cls:10s}): {history['pnn_digit_accs'][i] * 100:.2f}%\n")

    print(f"\n✓ Results saved to '{output_dir}/'")
    print(f"  Full precision accuracy : {history['computer_accuracy'] * 100:.2f}%")
    print(f"  PNN (LUT) accuracy    : {history['pnn_accuracy']      * 100:.2f}%")

    # 7. Machine-readable results for cross-script summary
    with open(os.path.join(output_dir, 'results.json'), 'w') as fh:
        json.dump({
            'task': task_name,
            'computer_accuracy': round(history['computer_accuracy'] * 100, 2),
            'pnn_accuracy': round(history['pnn_accuracy'] * 100, 2),
...
def run_image_task(task_name, num_epochs, output_dir, batch_size=64):
    ...
    pnn_acc, pnn_cm, pnn_per_class = evaluate(q_model, device, test_loader,
                                                  compute_details=True)
    print(f"  → PNN (LUT) Accuracy:    {pnn_acc * 100:.2f}%")
...
def main():
    parser = argparse.ArgumentParser(
        description='PNN Scene Recognition – replicate paper results with hardware LUT'
    )
    parser.add_argument(
        '--task',
        type=str,
    ...
    parser.add_argument('--epochs_mnist',   type=int, default=40,
                        help='Training epochs for MNIST (default: 15)')
    parser.add_argument('--epochs_fashion', type=int, default=60,
                        help='Training epochs for Fashion-MNIST (default: 20)')
    parser.add_argument('--epochs_speech',  type=int, default=80,
